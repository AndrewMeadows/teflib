...
#define LOGUTIL_NOOP do{}while(0)
    ...
    #define LOGUTIL_TIMESTAMP std::cout << timing_util::get_local_datetime_string_with_msec() << " "
    #define LOGUTIL_TIMESTAMP_LEVEL(X) std::cout << timing_util::get_local_datetime_string_with_msec() << " (" << X << ") "
#else
...
//     At a minimum you must do: LOG("foo %s", bar)
#define LOG(fmtstr, ...) LOGUTIL_TIMESTAMP;std::printf(fmtstr, __VA_ARGS__)

#ifdef LOGUTIL_ENABLE_VERBOSITY
    #define LOG1(fmtstr, ...) if(log_util::get_verbosity()>0){LOGUTIL_TIMESTAMP_LEVEL(1);std::printf(fmtstr, __VA_ARGS__);}
    #define LOG2(fmtstr, ...) if(log_util::get_verbosity()>1){LOGUTIL_TIMESTAMP_LEVEL(2);std::printf(fmtstr, __VA_ARGS__);}
    #define LOG3(fmtstr, ...) if(log_util::get_verbosity()>2){LOGUTIL_TIMESTAMP_LEVEL(3);std::printf(fmtstr, __VA_ARGS__);}
#else
